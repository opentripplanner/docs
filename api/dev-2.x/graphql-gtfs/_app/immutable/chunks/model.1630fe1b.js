...
`;class WT{constructor(s){tu(this,"mapping");this.mapping=s}getFor(s){return this.mapping.get(s.name)}}var Si;(function(t){t.UNION="union",t.ARGUMENT="argument",t.FIELD="field"})(Si||(Si={}));function YT(t){const s=new Map;return $T(t,r=>{Ke(r)&&HT(r,o=>{const l=pa(o.type);la(s,l).references.push({kind:Si.FIELD,parent:r,by:o}),jT(o,g=>{const T=pa(g.type);la(s,T).references.push({kind:Si.ARGUMENT,field:o,type:r,by:g})})}),Ln(r)&&r.getTypes().forEach(o=>{la(s,o).references.push({kind:Si.UNION,by:r})})}),new WT(s)}function $T(t,s){Object.entries(t.getTypeMap()).forEach(([,r])=>{r.name.startsWith("__")||s(r)})}function HT(t,s){Object.entries(t.getFields()).forEach(([,r])=>{s(r)})}function jT(t,s){Object.entries(t.args).forEach(([,r])=>{s(r)})}function la(t,s){let r=t.get(s.name);return r||(r={references:[]},t.set(s.name,r)),r}function pa(t){return $e(t)||xt(t)?pa(t.ofType):t}const je=rI(),ga=ny(jv.DIRECTIVES,[]);var yu;const qT=Ur((yu=je.getQueryType())==null?void 0:yu.getFields());var Tu;const QT=Ur((Tu=je.getMutationType())==null?void 0:Tu.getFields());var Iu;const JT=Ur((Iu=je.getSubscriptionType())==null?void 0:Iu.getFields()),wa=Tn.keyBy(rc(),t=>t.name.toLocaleLowerCase()),zT=Tn.mapValues(wa,t=>{const s=ga.find(r=>(r==null?void 0:r.name)===t.name||(r==null?void 0:r.name)==="*");return s?s.args.some(r=>r==="*")?t.args:s.args.map(r=>t.args.find(o=>o.name===r)).filter(r=>!!r):[]}),XT=Ur(je.getTypeMap()),KT=YT(je);function Ur(t){return Tn.mapKeys(t||{},(s,r)=>r.toLocaleLowerCase())}function ZT(){return Tn.size(je.getTypeMap())<=10}function fI(){return[ua("Queries",je.getQueryType()),ua("Mutations",je.getMutationType()),ua("Subscriptions",je.getSubscriptionType()),sI(),tI()].filter(t=>!!t)}function ua(t,s){return nI(t,eI(s))}function eI(t){return Tn.sortBy((t==null?void 0:t.getFields())||{},s=>s.name)}function nI(t,s){return s.length===0?null:{type:"menu",title:t,children:s.map(r=>({type:"page",title:r.name,section:t,deprecated:!!r.deprecationReason,href:ma.joinUrlPaths(va,t.toLocaleLowerCase(),r.name)}))}}function tI(){return ZT()?null:{type:"menu",title:"Types",children:Tn.sortBy(Tn.map(je.getTypeMap()),s=>s.name).filter(s=>!s.name.startsWith("__")).map(s=>({type:"page",title:s.name,section:"Types",href:ma.joinUrlPaths(va,"types",s.name)}))}}function hI(){return!!je.getQueryType()}function dI(t){return qT[t.toLocaleLowerCase()]}function pI(){return!!je.getMutationType()}function gI(t){return QT[t.toLocaleLowerCase()]}function mI(){return!!je.getSubscriptionType()}function vI(t){return JT[t.toLocaleLowerCase()]}function yI(t){return XT[t.toLocaleLowerCase()]}function iI(t){return wa[t.toLocaleLowerCase()]}function TI(t){return iI(t.name)!==void 0}function II(){return Tn.size(wa)>0}function bI(t){return zT[t.name.toLocaleLowerCase()]||[]}function EI(t){if(t)return KT.getFor(t)}function wI(t){return Tn.flatMap(t.getFields(),s=>({field:s,possibleDescriptions:ic(s,t)}))}function ic(t,s){return t?t.description?[{description:t.description,from:s}]:Ke(s)?s.getInterfaces().flatMap(r=>ic(r.getFields()[t.name],r)):[]:[]}function rc(){return ga.some(t=>(t==null?void 0:t.name)==="*")?je.getDirectives().filter(t=>!["include","skip","deprecated","specifiedBy"].includes(t.name)):ga.filter(t=>!!(t!=null&&t.name)).map(({name:t})=>t?je.getDirective(t):void 0).filter(t=>!!t)}function rI(){return vu.trim().length===0?BT(JSON.parse(JSON.stringify({__schema:{types:[]}}))):VT(vu)}function sI(){const t=rc();return t.length===0?null:{type:"menu",title:"Directives",children:t.map(s=>({type:"page",title:s.name,href:ma.joinUrlPaths(va,"directives",s.name),section:"Directives"}))}}export{Ke as A,Cn as B,wI as C,jy as D,Uu as E,uI as F,y as K,Si as R,pI as a,gI as b,hI as c,dI as d,mI as e,vI as f,iI as g,II as h,ZT as i,yI as j,EI as k,ny as l,fI as m,bI as n,TI as o,Bu as p,xt as q,$e as r,je as s,jv as t,cI as u,Tn as v,vt as w,kn as x,He as y,Ln as z};
